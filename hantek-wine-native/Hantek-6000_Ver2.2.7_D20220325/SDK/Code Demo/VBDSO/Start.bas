...
Dim i As Long
    ...
    stControl.nTriggerSource = 1 '触发源
    stControl.nHTriggerPos = 50 '水平触发位置
    stControl.nVTriggerPos = LeverPos(1) '垂直触发位置
...
Public Sub InitHard()
    ...
    result = dsoHTSetSampleRate(DeviceIndex, YTFormat, rcRelayControl, stControl)  '设置采样率
    ...
    For i = 0 To 3
        result = dsoHTSetCHPos(DeviceIndex, rcRelayControl.nCHVoltDIV(i), LeverPos(i), i, 4)  '设置通道垂直位置
    Next i
