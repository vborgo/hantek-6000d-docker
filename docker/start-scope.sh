#!/bin/bash
# Launch Scope.exe after Wine's PnP has auto-loaded the Oscilloscope driver.
# With the driverstore populated, plugplay.exe loads the driver with the real USB PDO
# (via AddDevice) — not the orphan device that sc start alone would create.

sleep 2  # Wait for Xvfb

# Start the Wine server by touching the USB registry tree
...
echo "start-scope: waiting for Hantek USB device to appear in Wine..."
INSTANCE=""
...
if [ -n "$INSTANCE" ]; then
    ...
    echo "start-scope: WARNING — Hantek USB device not visible in Wine after 30s (not connected?)"
fi
...
if [ "$STATE" != "4" ]; then
    # PnP didn't auto-load — fall back to sc start (may create orphan device, but better than nothing)
